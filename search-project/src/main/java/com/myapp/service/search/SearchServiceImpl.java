...
import com.myapp.base.HouseSort;
import com.myapp.base.HouseStatus;
import com.myapp.base.RentValueBlock;
import com.myapp.entity.House;
import com.myapp.entity.HouseDetail;
import com.myapp.entity.HouseTag;
import com.myapp.entity.SupportAddress;
import com.myapp.repository.HouseDetailRepository;
import com.myapp.repository.HouseRepository;
import com.myapp.repository.HouseTagRepository;
import com.myapp.repository.SupportAddressRepository;
...
public class SearchServiceImpl implements ISearchService {
    ...
    private static final String INDEX_TOPIC = "search";
    ...
    private void createOrUpdate(HouseIndexMessage message) {
        ...
        if (house == null){
            logger.error("index house {} don't exist! ", houseId);
        ...
        SearchResponse searchRespons = requestBuilder.get();

        boolean success;
        long totalHit = searchRespons.getHits().getTotalHits();
        if (totalHit == 0){
            success = create(indexTemplate);
        }else if (totalHit == 1){
            String esId = searchRespons.getHits().getAt(0).getId();
    ...
    public ServiceMultiResult<Long> query(RentSearch rentSearch) {
        ...
        if (!RentValueBlock.ALL.equals(price)){
            RangeQueryBuilder rangeQueryBuilder1 = QueryBuilders.rangeQuery(HouseIndexKey.PRICE);
            if (price.getMax() > 0){
                rangeQueryBuilder1.lte(price.getMax());
            }
            if (price.getMin() > 0){
                rangeQueryBuilder1.gte(price.getMin());
            }
            boolQuery.filter(rangeQueryBuilder1);
        }
        ...
        boolQuery.should(QueryBuilders.multiMatchQuery(rentSearch.getKeywords(),
                HouseIndexKey.TRAFFIC,HouseIndexKey.DISTRICT,
    ...
    public ServiceResult<List<String>> suggest(String prefix) {
        ...
        SearchRequestBuilder requestBuilder = this.client.prepareSearch(INDEX_NAME).setTypes(INDEX_TYPE).suggest(suggestBuilder);
        logger.debug(requestBuilder.toString());
        ...
        List<String> res = Lists.newArrayList(suggestSet.toArray(new String[]{}));
    ...
    */
    ...
    public ServiceResult<Long> aggregateDistrictHouse(String cityEnName, String regionEnName, String district) {
        ...
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery().filter(QueryBuilders.termQuery(HouseIndexKey.CITY_EN_NAME, cityEnName))
                                    .filter(QueryBuilders.termQuery(HouseIndexKey.REGION_EN_NAME, regionEnName))
                                    .filter(QueryBuilders.termQuery(HouseIndexKey.DISTRICT, district));

        SearchRequestBuilder requestBuilder = this.client.prepareSearch(INDEX_NAME).setTypes(INDEX_TYPE)
                .setQuery(boolQuery).addAggregation(
    ...
    */
    ...
    public ServiceMultiResult<HouseBucketDTO> mapAggregate(String cityEnName) {
        ...
        SearchRequestBuilder requestBuilder = this.client.prepareSearch(INDEX_NAME).setTypes(INDEX_TYPE).setQuery(boolQuery)
                .addAggregation(
                        AggregationBuilders.terms(HouseIndexKey.AGG_REGION)
                        .field(HouseIndexKey.CITY_EN_NAME)
                );
        SearchResponse response = requestBuilder.get();
    ...
    public ServiceMultiResult<Long> mapQuery(String cityEnName, String orderBy, String orderDirection, int start, int size) {
        ...
        SearchRequestBuilder searchRequestBuilder = this.client.prepareSearch(INDEX_NAME).setTypes(INDEX_TYPE)
                .setQuery(boolQuery)
    ...
    @Override
    public ServiceMultiResult<Long> mapQuery(MapSearch search) {
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
        boolQuery.filter(QueryBuilders.termQuery(HouseIndexKey.CITY_EN_NAME, search.getCityEnName()));

        boolQuery.filter(
                QueryBuilders.geoBoundingBoxQuery("location")
                .setCorners(
                        new GeoPoint(search.getLeftLatitude(), search.getLeftLongitude()),
                                new GeoPoint(search.getRightLatitude(), search.getRightLongitude())
                )
        );
        SearchRequestBuilder searchRequestBuilder = this.client.prepareSearch(INDEX_NAME).setTypes(INDEX_TYPE)
                .setQuery(boolQuery)
                .addSort(HouseSort.getSortKey(search.getOrderBy()), SortOrder.fromString(search.getOrderDirection()))
    ...
    private boolean updateSuggest(HouseIndexTemplate indexTemplate){
        AnalyzeRequestBuilder requestBuilder = new AnalyzeRequestBuilder(this.client, AnalyzeAction.INSTANCE,INDEX_NAME,
    ...
    private void remove(Long houseId, int retry){
        ...
        try {
            ...
            logger.error("cannot encode json for " + message);
        }
    }
}
